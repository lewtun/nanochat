...
Once you kick off the run, you wait ~1.5 hours. Trackio stores named runs locally by default; launch the dashboard with `trackio show --project nanochat`. You can also inspect final metrics from the terminal, for example:

```
trackio get metric --project nanochat --run d26-feb2-fp8-ratio8.25 --metric core_metric
trackio get metric --project nanochat --run d26-feb2-fp8-ratio8.25 --metric total_training_flops
trackio get metric --project nanochat --run d26-feb2-fp8-ratio8.25 --metric total_training_time
```

Set `TRACKIO_SPACE_ID` before training to log to a shared Hugging Face Space instead. Trackio creates the Space when needed, provided you have authenticated with `hf auth login` and have write access to its namespace.
...
core_metric 0.25851
step 16704
total_training_flops 4.330784131228946e+19
total_training_time 10949.46713
```
